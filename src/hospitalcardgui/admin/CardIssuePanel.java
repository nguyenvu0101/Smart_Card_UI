...
public class CardIssuePanel extends JPanel {
    ...
    private void initUI() {
        setBorder(BorderFactory.createTitledBorder("Cấp thẻ bệnh nhân mới"));
        ...
        int row = 0;
        form.add(new JLabel("Mã bệnh nhân (patient_id):"), at(base,0,row));
        ...
        form.add(new JLabel("Mã thẻ (card_id):"), at(base,0,row));
        form.add(txtCardId = new JTextField(20), at(base,1,row++));

        add(form, BorderLayout.CENTER);

        JPanel bottom = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        JButton btnSave = new JButton("Lưu & Cấp thẻ");
    ...
    private void onSave() {
        ...
        if (patientId.isEmpty() || cardId.isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    "Bắt buộc nhập patient_id và card_id.");
            return;
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            // 1. Insert vào patients
            String sqlPatient =
                    "INSERT INTO patients (" +
                    "patient_id, full_name, date_of_birth, home_address, " +
...
if (dobStr.isEmpty()) {
    ...
    pst.setDate(3, dob);
            ...
            // 2. Insert vào smartcards
            String sqlCard =
                    "INSERT INTO smartcards (" +
                    "card_id, patient_id, card_status" +
                    ") VALUES (?, ?, 'ACTIVE')";
            ...
            conn.commit();
            JOptionPane.showMessageDialog(this,
                    "Đã lưu thông tin bệnh nhân và cấp thẻ thành công.");
            clearForm();

        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this,
                    "Lỗi khi lưu vào Supabase: " + ex.getMessage(),
